...
In this CLI, Better Auth for Next.js is generated with a database adapter
...
lib/
  ...
  prisma.ts           # Prisma client (when using Prisma)
  db.ts               # Drizzle client (when using Drizzle)
app/
...
<Callout type="tip">
  Prisma + Better Auth generates Better Auth tables in `prisma/schema.prisma`.
  Drizzle + Better Auth generates them in `lib/schema.ts`.
</Callout>
