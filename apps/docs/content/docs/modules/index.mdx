...
>
  Next.js 16 or TanStack Start with React 19 and Tailwind CSS v4 - selected via
  the full-stack framework prompt
</Card>
...
>
  JWT + bcrypt for Express, Better Auth for Next.js and TanStack Start
  (requires a database)
</Card>
...
- TanStack Start: new routes -> `src/routes/` (then run `npm run build` to regenerate `routeTree.gen.ts`), server functions -> `createServerFn`, components -> `src/components/`
