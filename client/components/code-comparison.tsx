			...
			<div className="container mx-auto px-4">
				<div className="text-center mb-16">
					...
					<p className="text-zinc-500 max-w-2xl mx-auto">
						See the difference. Nexus bridges the gap between your backend and frontend.
					</p>
				</div>
				...
				<div className="flex flex-col lg:flex-row gap-8 lg:gap-0 max-w-5xl mx-auto rounded-xl overflow-hidden shadow-2xl border border-zinc-200 dark:border-zinc-800">
					...
					<div className="flex-1 bg-[#1e1e1e] p-6 lg:border-r border-zinc-700">
						<div className="text-xs font-mono text-zinc-500 mb-4 flex items-center justify-between">
							<span>traditional-fetch.ts</span>
						...
						<pre className="font-mono text-sm leading-relaxed overflow-x-auto text-zinc-300">
							<code>
								...
								fetch(<span className="text-[#ce9178]">&apos;/api/users&apos;</span>).then(r =&gt; r.json())
								{"\n\n"}
								<span className="text-zinc-500">{"// ❌ No type inference"}</span>
								{"\n"}
								console.log(data.user.
								<span className="text-red-400 underline decoration-wavy underline-offset-2">nmae</span>)
							</code>
						</pre>
					</div>
					...
					<div className="flex-1 bg-[#1e1e1e] p-6 relative">
						<div className="text-xs font-mono text-zinc-500 mb-4 flex items-center justify-between">
							<span>nexus-client.ts</span>
							<span className="text-emerald-400 flex items-center gap-1">
								<Check size={12} /> Type-Safe
							</span>
						</div>
						<pre className="font-mono text-sm leading-relaxed overflow-x-auto text-zinc-300">
							<code>
									...
									<AnimatePresence>
										{hoveredToken === "get" && (
											<motion.div
												...
												<div className="text-[#569cd6] mb-1">method getUsers(): Promise&lt;User[]&gt;</div>
												<div className="text-zinc-400">Go to definition (Ctrl + Click)</div>
											</motion.div>
										)}
									</AnimatePresence>
								</span>
