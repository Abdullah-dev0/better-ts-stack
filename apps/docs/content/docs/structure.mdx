...
A TanStack Start project is a **single Vite-based application** at the project root. File-based routing lives in `src/routes/`; the route tree (`src/routeTree.gen.ts`) is generated by the CLI during build.
...
| `src/routes/routeTree.gen.ts` | Generated route tree (regenerated by `vite build`)      |
