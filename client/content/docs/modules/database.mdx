...
<Callout type="info">
  In Next.js projects, Prisma reads `DATABASE_URL` from `prisma.config.ts`.
  If Better Auth is enabled, `schema.prisma` includes Better Auth core tables.
</Callout>
...
```bash
# Generated in package.json by Prisma module
...
<Callout type="tip">
  If you also enable **Better Auth** with Drizzle, the generated `lib/schema.ts`
  includes Better Auth core tables (`user`, `session`, `account`,
  `verification`) instead of only the sample `users` table.
</Callout>
