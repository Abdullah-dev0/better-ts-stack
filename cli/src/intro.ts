...
export async function showInteractiveIntro() {
  ...
  const titleLines = [titleLine1, titleLine2, titleLine3, titleLine4, titleLine5];

  const centerText = (text: string) => {
    const textLen = stripAnsi(text).length;
    const padding = Math.floor((width - 2 - textLen) / 2);
    return ' '.repeat(padding) + text + ' '.repeat(width - 2 - textLen - padding);
  };

  // Animate title appearance
  for (const line of titleLines) {
    process.stdout.write('\r' + borderSide + centerText(line) + borderSide + '\n');
    await sleep(80);
  }

  // Show subtitle
  process.stdout.write('\r' + borderSide + centerText(bold + subtitle + reset) + borderSide + '\n');
  await sleep(200);
  ...
  await typewriter(
    brightGreen + 'Build powerful TypeScript projects with enterprise-grade tooling' + reset,
    15
  );
  ...
  const features = [
    { icon: '⚡', color: brightYellow, text: 'Lightning-fast setup with zero configuration' },
    { icon: '🎯', color: brightCyan, text: 'Production-ready templates for every use case' },
    { icon: '🛠️ ', color: brightMagenta, text: 'Modern tooling: ESLint, Prettier, Vitest & more' },
    { icon: '📦', color: brightGreen, text: 'Optimized builds with best practices baked in' },
  ];
