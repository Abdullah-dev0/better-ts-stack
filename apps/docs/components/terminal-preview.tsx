...
const SETUP_SCRIPT: LineKind[] = [
  ...
  {
    kind: "output",
    text: "✓  Express + TypeScript configured",
    color: "text-emerald-400",
  },
  {
    kind: "output",
    text: "✓  Prisma schema generated",
    color: "text-emerald-400",
  },
  {
    kind: "output",
    text: "✓  Next.js 16 + Tailwind ready",
    color: "text-emerald-400",
  },
  {
    kind: "output",
    text: "✓  Docker Compose configured",
    color: "text-emerald-400",
  },
...
function ComputedLine({ line }: { line: LineKind }) {
  ...
  if (line.kind === "cmd")
    return (
      <div className="flex items-center gap-2">
        ...
        <span className="font-mono text-sm text-(--terminal-text,#e2e8f0)">
          {line.text}
        </span>
      </div>
    );
  if (line.kind === "prompt")
    return (
      <div className="flex flex-wrap gap-0 font-mono text-xs">
  ...
  return (
    <div
      className={`font-mono text-xs ${line.color ?? "text-muted-foreground"}`}
    >
      {line.text}
    </div>
  );
...
export const TerminalPreview = () => {
  ...
  const [phase, setPhase] = useState<"typing-cmd" | "typing-answer" | "idle">(
    "typing-cmd"
  );
      ...
      return (
        <div className="flex items-center gap-2">
          ...
          <MotionDiv
            ...
            className="bg-primary inline-block h-4 w-1.5 align-middle"
          />
        </div>
      );
    }
  ...
  return (
    ...
    >
      <MotionDiv
        ...
        className="glass-panel terminal-glow relative overflow-hidden rounded-xl"
      >
        {/* Header */}
        ...
        <div
          ref={terminalBodyRef}
          className="bg-terminal-bg scrollbar-thin scrollbar-track-transparent scrollbar-thumb-border max-h-[340px] min-h-[220px] space-y-0.5 overflow-y-auto p-5"
        >
        ...
        >
          <button
            onClick={handleCopy}
            title="Copy command"
            className="bg-muted/50 hover:bg-muted group absolute top-14 right-4 rounded-lg p-2 transition-colors"
          >
            <MotionDiv
